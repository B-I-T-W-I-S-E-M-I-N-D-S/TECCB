...
#     'video_font_path': './data/Poppins Medium Italic 500.ttf',  # Primary font
#     'video_font_fallback': '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',  # System font fallback
#     'video_pred_text_y': 0.45,
#     'video_gt_text_y': 0.55,
#     'video_footer_height': 100,  # Pixels for bars below video
#     'video_gt_bar_y': 0.3,  # Relative to footer
#     'video_pred_bar_y': 0.6,  # Relative to footer
#     'video_bar_height': 0.2,  # Relative to footer
#     'video_bar_text_scale': 0.4,  # Smaller text for bars
# }
...
# ) -> None:
#     """
#     Annotate a video with predicted and ground truth action labels and real-time bars below the frame.
...
#         except IOError:
#             print(f"Warning: Neither {font_path} nor {font_fallback} found. Falling back to OpenCV default font.")
#             font = None
#             bar_font = None
...
#         # Draw GT and prediction bars in footer
...
#                 start_x = int((seg['start'] / duration) * frame_width)
#                 end_x = int((seg['end'] / duration) * frame_width)
#                 cv2.rectangle(
#                     extended_frame,
#                     (start_x, gt_bar_y),
...
#                     )
#                     draw.text((start_x, gt_bar_y - 10), f"{seg['start']:.1f}", font=bar_font, fill=gt_color_rgb)
#                     draw.text((end_x, gt_bar_y - 10), f"{seg['end']:.1f}", font=bar_font, fill=gt_color_rgb)
...
#                         fill=(255, 255, 255)
#                     )
#                     draw.text((start_x, pred_bar_y + bar_height + 10), f"{seg['start']:.1f}", font=bar_font, fill=pred_color_rgb)
#                     draw.text((end_x, pred_bar_y + bar_height + 10), f"{seg['end']:.1f}", font=bar_font, fill=pred_color_rgb)
...
VIS_CONFIG = {
    ...
    'video_pred_text_color': (14, 127, 255),  # BGR for OpenCV
    'video_text_thickness': 1,
    'video_font_path': "./data/Poppins Bold Italic 700.ttf",  # Default to None, use fallback
    'video_font_fallback': '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',  # System font
    'video_pred_text_y': 0.45,
    'video_gt_text_y': 0.55,
    'video_footer_height': 100,
    'video_gt_bar_y': 0.3,
    'video_pred_bar_y': 0.6,
    'video_bar_height': 0.2,
    'video_bar_text_scale': 0.4,
    'min_segment_duration': 1.0,  # Filter segments shorter than 1s
}
...
def annotate_video_with_actions(
    ...
    """
    Annotate a video with predicted and ground truth action labels and cumulative bars below the frame.
    ...
    while cap.isOpened():
        ...
        # Draw GT and prediction bars in footer (cumulative up to timestamp)
        footer_y = frame_height
        gt_bar_y = footer_y + int(VIS_CONFIG['video_gt_bar_y'] * footer_height)
        pred_bar_y = footer_y + int(VIS_CONFIG['video_pred_bar_y'] * footer_height)
        bar_height = int(VIS_CONFIG['video_bar_height'] * footer_height)
        ...
        for seg in gt_segments:
            if seg['start'] <= timestamp:
                start_x = int((seg['start'] / duration) * frame_width)
                end_x = int((min(seg['end'], timestamp) / duration) * frame_width)
                cv2.rectangle(
                    extended_frame,
                    (start_x, gt_bar_y),
                    (end_x, gt_bar_y + bar_height),
        ...
        if font:
            ...
            for seg in gt_segments:
                if seg['start'] <= timestamp:
                    ...
                    start_x = int((seg['start'] / duration) * frame_width)
                    end_x = int((min(seg['end'], timestamp) / duration) * frame_width)
                    if end_x - start_x >= 20:  # Skip labels for very short segments
                        draw.text(
                            ((start_x + end_x) / 2, gt_bar_y + bar_height / 2),
                            label,
                            font=bar_font,
                        ...
                        draw.text((start_x, gt_bar_y - 10), f"{seg['start']:.1f}", font=bar_font, fill=gt_color_rgb)
                        draw.text((end_x, gt_bar_y - 10), f"{min(seg['end'], timestamp):.1f}", font=bar_font, fill=gt_color_rgb)
            ...
            for seg in pred_segments:
                if seg['start'] <= timestamp:
                    ...
                    if end_x - start_x >= 20:
                        ...
                        draw.text((start_x, pred_bar_y + bar_height + 10), f"{seg['start']:.1f}", font=bar_font, fill=pred_color_rgb)
                        draw.text((end_x, pred_bar_y + bar_height + 10), f"{min(seg['end'], timestamp):.1f}", font=bar_font, fill=pred_color_rgb)
