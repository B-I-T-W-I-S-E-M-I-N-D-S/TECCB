...
def train(opt): 
    ...
    # Initialize best_map to ensure the first valid mAP is saved
    model.best_map = 0.0  # Initialize to 0.0 to allow any positive mAP to trigger saving
    ...
    for n_epoch in range(opt['epoch']):   
        ...
        # Save the best checkpoint if IoUmAP_5 is better than or equal to the current best_map
        if IoUmAP_5 >= model.best_map:  # Use >= to ensure at least one checkpoint is saved
            model.best_map = IoUmAP_5
            torch.save(state, opt["checkpoint_path"]+"/"+opt["exp"]+"_ckp_best.pth.tar")
            print(f"Saved best checkpoint at epoch {n_epoch+1} with mAP: {IoUmAP_5}")
...
def test_frame(opt): 
    ...
    print("working time : {}s, %s frames".format(working_time, total_frames/working_time, total_frames))
...
def test_online(opt): 
    ...
    for video_name in dataset.video_list:
        ...
        for idx in range(0,duration):
            ...
            sup_queue[:-1,:]=sup_queue[1:simulate
:,:].clone()
            sup_queue[-1,:]=0
            for proposal in proposal_anc_dict:
                cls_idx = dataset.label_name.index(proposal['label'])
    ...
    output_dict={"version":"VERSION 1.3","results":result_dict,"string":{}}
    outfile=open(opt["result_file"].format(opt['exp']),"w")
    json.dump(output_dict,outfile, indent=2)
    outfile.close()
