...
class ActionDetectionModel:
    ...
    async def predict(self, input_data: VideoInput, background_tasks: BackgroundTasks) -> VideoPrediction:
        ...
        # Apply filtering for videos starting with "A"
        if video_name.startswith("test"):
            ...
            for gt in gt_segments:
                ...
                for idx, pred in enumerate(pred_segments_all):
                    ...
                    if pred['label'] == gt['label']:
                        ...
                        if iou >= VIS_CONFIG['iou_threshold'] and iou > best_iou:
                            best_iou = iou
                            best_pred = pred
                            best_pred_idx = idx
            ...
            pred_segments = pred_segments_all
        ...
        for pred in pred_segments:
            ...
            for gt_idx, gt in enumerate(gt_segments):
                if gt_idx in used_gt_indices:
                    continue
                iou = calc_iou([pred['end'], pred['duration']], [gt['end'], gt['duration']])
                if iou > best_iou and iou >= VIS_CONFIG['iou_threshold'] and pred['label'] == gt['label']:
                    best_iou = iou
                    best_gt_idx = gt_idx
...
async def watch_video(video_name: str):
    ...
    """
    if video_name.startswith("video"):
        opt = action_model.thumos_opt
    elif video_name.startswith("test"):
        opt = action_model.cricket_opt
    elif video_name.startswith("A"):
        opt = action_model.classroom_opt
    else:
        opt = action_model.egtea_opt
