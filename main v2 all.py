...
#     #for thumos dataset only. 1 line
#     opt["split"] = "train"
...
# def fix_tensor_shapes(model_output, target_label, tensor_name=""):
#     """
#     Fix tensor shape mismatches between model output and target labels
#     """
#     print(f"DEBUG - Fixing {tensor_name} shapes:")
#     print(f"  Model output shape: {model_output.shape}")
#     print(f"  Target label shape: {target_label.shape}")
    
#     # Handle different dimension mismatches
#     if len(model_output.shape) != len(target_label.shape):
#         print(f"  ERROR: Dimension count mismatch - output: {len(model_output.shape)}, target: {len(target_label.shape)}")
#         return model_output, target_label
    
#     # Fix each dimension if there's a mismatch
#     fixed_output = model_output
#     fixed_target = target_label
    
#     for dim in range(len(model_output.shape)):
#         if model_output.shape[dim] != target_label.shape[dim]:
#             min_size = min(model_output.shape[dim], target_label.shape[dim])
#             print(f"  Fixing dimension {dim}: {model_output.shape[dim]} vs {target_label.shape[dim]} -> {min_size}")
            
#             # Create slicing tuple
#             output_slice = [slice(None)] * len(model_output.shape)
#             target_slice = [slice(None)] * len(target_label.shape)
            
#             output_slice[dim] = slice(0, min_size)
#             target_slice[dim] = slice(0, min_size)
            
#             fixed_output = fixed_output[tuple(output_slice)]
#             fixed_target = fixed_target[tuple(target_slice)]
    
#     print(f"  Fixed shapes - output: {fixed_output.shape}, target: {fixed_target.shape}")
#     return fixed_output, fixed_target
...
#         # Move input to GPU and ensure correct dtype
...
#         # DEBUG: Print tensor shapes to identify the mismatch
#         if n_iter == 0:  # Only print for first iteration to avoid spam
#             print(f"DEBUG - Iteration {n_iter}:")
#             print(f"  act_cls shape: {act_cls.shape}")
#             print(f"  cls_label shape: {cls_label.shape}")
#             print(f"  snip_cls shape: {snip_cls.shape}")
#             print(f"  snip_label shape: {snip_label.shape}")
        
#         # Fix shape mismatches
#         act_cls, cls_label = fix_tensor_shapes(act_cls, cls_label, "classification")
#         snip_cls, snip_label = fix_tensor_shapes(snip_cls, snip_label, "snippet")
...
#     # DEBUG: Print model's expected number of classes
#     print(f"DEBUG - Model expects {opt.get('num_of_class', 'UNKNOWN')} classes")
...
#     # DEBUG: Check dataset class information
#     print(f"DEBUG - Dataset label names: {len(getattr(train_dataset, 'label_name', []))}")
#     if hasattr(train_dataset, 'label_name'):
#         print(f"DEBUG - Label names: {train_dataset.label_name}")
    
#     # Sample a few batches to understand the data structure
#     print("DEBUG - Sampling data to understand tensor shapes...")
#     sample_loader = torch.utils.data.DataLoader(train_dataset, batch_size=2, shuffle=False, num_workers=0)
#     for i, (input_data, cls_label, reg_label, snip_label) in enumerate(sample_loader):
#         print(f"  Sample {i}: input={input_data.shape}, cls={cls_label.shape}, reg={reg_label.shape}, snip={snip_label.shape}")
...
#     for n_iter,(input_data,cls_label,reg_label, _) in enumerate(tqdm(test_loader)):
#         # Move to GPU and get model outputs
#         input_data = input_data.float().cuda()
#         cls_label = cls_label.cuda()
#         reg_label = reg_label.cuda()
...
#         # Apply same shape fixing as in training
#         act_cls, cls_label = fix_tensor_shapes(act_cls, cls_label, "eval_classification")
