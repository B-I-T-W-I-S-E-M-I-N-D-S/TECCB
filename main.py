...
def train_one_epoch(opt, model, train_dataset, optimizer, warmup=False):
    ...
    for n_iter, (input_data, cls_label, reg_label, snip_label) in enumerate(tqdm(train_loader)):
        ...
        input_data = input_data.float().cuda(non_blocking=True)
...
def train(opt): 
    ...
    # Move model to GPU and enable DataParallel if multiple GPUs are available
    ...
    optimizer = optim.Adam([
        {'params': model.module.history_unit.parameters() if isinstance(model, nn.DataParallel) else model.history_unit.parameters(), 'lr': 1e-6},
    ...
    for n_epoch in range(opt['epoch']):   
        ...
        torch.save(state, os.path.join(opt["checkpoint_path"], f"{opt['exp']}_checkpoint_{n_epoch + 1}.pth.tar"))
        if IoUmAP_5 > (model.module.best_map if isinstance(model, nn.DataParallel) else model.best_map):
    ...
    writer.close()
    return model.module.best_map if isinstance(model, nn.DataParallel) else model.best_map
...
def eval_map_supnet(opt, dataset, output_cls, output_reg, labels_cls, labels_reg):
    ...
    checkpoint = torch.load(os.path.join(opt["checkpoint_path"], "ckp_best_suppress.pth.tar"))
...
def test_frame(opt): 
    ...
    checkpoint = torch.load(os.path.join(opt["checkpoint_path"], "ckp_best.pth.tar"))
    base_dict = checkpoint['state_dict']
    if isinstance(model, nn.DataParallel):
        model.module.load_state_dict(base_dict)
    else:
...
def test(opt): 
    ...
    checkpoint = torch.load(os.path.join(opt["checkpoint_path"], f"{opt['exp']}_ckp_best.pth.tar"))
    base_dict = checkpoint['state_dict']
    if isinstance(model, nn.DataParallel):
        model.module.load_state_dict(base_dict)
    else:
